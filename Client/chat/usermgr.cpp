...
void UserMgr::AppendMaybeList(QJsonArray array)
{
    // 遍历 QJsonArray 并输出每个元素
    for (const QJsonValue& value : array) {
        auto name = value["name"].toString();
        ...
        auto sex = value["sex"].toInt();
        auto uid = value["uid"].toInt();
        auto score = value["score"].toDouble();

        auto info = std::make_shared<MaybeInfo>(uid, name, desc,
             icon, nick, sex, score);
        _maybe_list.push_back(info);
    }
}
...
std::vector<std::shared_ptr<MaybeInfo> > UserMgr::GetMaybeList()
{
    return _maybe_list;
}
