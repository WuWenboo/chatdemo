    ...
    : QListWidget(parent), _load_pending(false)
{
    ...
    addContactUserList();
    //连接点击的信号和槽
    QObject::connect(this, &QListWidget::itemClicked, this, &ContactUserList::slot_item_clicked);
    //链接对端同意认证后通知的信号
    QObject::connect(TcpMgr::GetInstance().get(), &TcpMgr::sig_add_auth_friend,this, &ContactUserList::slot_add_auth_firend);
    //链接自己点击同意认证后界面刷新
    QObject::connect(TcpMgr::GetInstance().get(), &TcpMgr::sig_auth_rsp,this, &ContactUserList::slot_auth_rsp);
}
...
void ContactUserList::addContactUserList()
{
    ...
    _con_maybe_friend_item = new ConUserItem();
    _con_maybe_friend_item->setObjectName("maybe_friend_item");
    _con_maybe_friend_item->SetInfo(0, tr("可能认识的人"), ":/res/maybe_friend.png");
    _con_maybe_friend_item->SetItemType(ListItemType::MAYBE_FRIEND_ITEM);

    QListWidgetItem *maybe_item = new QListWidgetItem;
    maybe_item->setSizeHint(_con_maybe_friend_item->sizeHint());
    this->addItem(maybe_item);
    this->setItemWidget(maybe_item, _con_maybe_friend_item);
    ...
    // 创建QListWidgetItem，并设置自定义的widget(test)
    for(int i = 0; i < 13; i++){
...
void ContactUserList::slot_item_clicked(QListWidgetItem *item)
{
   ...
   if(itemType == ListItemType::MAYBE_FRIEND_ITEM){
       // 创建对话框，提示用户
       qDebug() << "maybe friend item clicked ";
       //跳转到可能认识的人界面
       emit sig_switch_maybe_friend_page();
   }
