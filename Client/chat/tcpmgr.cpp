...
//           #define forever for(;;) Qt提供forever宏
           forever {
                //先解析头部
               if(!_b_recv_pending){
       ...
       // 处理错误(指向QTcpSocket函数的函数指针，返回值是void, 参数是Socketerror)
...
{
    //auto self = shared_from_this(); 错误，TcpMgr类没构造完成，不能传入lambda
    _handlers.insert(ID_CHAT_LOGIN_RSP, [this](ReqId id, int len, QByteArray data){
        ...
        //添加可能认识的人列表
        if (jsonObj.contains("maybe_list")) {
            UserMgr::GetInstance()->AppendMaybeList(jsonObj["maybe_list"].toArray());
        }

        emit sig_switch_chatdialog(user_info);
    });
