...
void HttpConnection::Start()
{
	...
	http::async_read(_socket, _buffer, _request, [self](beast::error_code ec, std::size_t bytes_transferred) {
		try {
			...
			//处理读到的数据
			boost::ignore_unused(bytes_transferred);	//http处理器不用做粘包处理
			self->HandleReq();
			self-> CheckDeadline();
		}
...
void HttpConnection::HandleReq()
{
	...
	if (_request.method() == http::verb::get) {
		...
		if (!success) {
			...
			_response.set(http::field::content_type, "text/plain");	//回应报头
	...
	_response.set(http::field::server, "GateServer");	//告诉对方具体服务器
	WriteResponse();
	return;
}
